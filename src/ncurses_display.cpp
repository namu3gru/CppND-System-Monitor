...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes, WINDOW* window, int n) {
  ...
  for (int i = 0; i < (int)processes.size(); ++i) {
    ...
    if(processes[i].Ram().compare("0") != 0)
    {
      if(processes[i].CpuUtilization() > 0)
      {     
        ...
        mvwprintw(window, row, command_column, processes[i].Command().substr(0, window->_maxx - 56).c_str());
      }
    }
  }
  (void) n;
...
void NCursesDisplay::Display(System& system, int n) {
  ...
  WINDOW* process_window = newwin(n , x_max -1, system_window->_maxy + 1, 0);
